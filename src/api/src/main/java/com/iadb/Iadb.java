...
import static com.iadb.IadbApiConstants.ATTACH_APPLICATION_API_VERSION;
import static com.iadb.IadbApiConstants.ATTACH_APPLICATION_PACKAGE_NAME;
import static com.iadb.IadbApiConstants.BIND_APPLICATION_PERMISSION_GRANTED;
import static com.iadb.IadbApiConstants.BIND_APPLICATION_SERVER_PATCH_VERSION;
import static com.iadb.IadbApiConstants.BIND_APPLICATION_SERVER_SECONTEXT;
import static com.iadb.IadbApiConstants.BIND_APPLICATION_SERVER_UID;
import static com.iadb.IadbApiConstants.BIND_APPLICATION_SERVER_VERSION;
import static com.iadb.IadbApiConstants.BIND_APPLICATION_SHOULD_SHOW_REQUEST_PERMISSION_RATIONALE;
import static com.iadb.IadbApiConstants.REQUEST_PERMISSION_REPLY_ALLOWED;
...
import com.iadb.server.IIadbBinderService;
...
public class Iadb {
    ...
    private static IIadbBinderService service;
    ...
    private static final IIadbBinderApplication IADB_APPLICATION = new IIadbBinderApplication.Stub() {
    ...
    private static boolean attachApplicationV13(IBinder binder, String packageName) throws RemoteException {
        ...
        args.putInt(ATTACH_APPLICATION_API_VERSION, IadbApiConstants.SERVER_VERSION);
        args.putString(ATTACH_APPLICATION_PACKAGE_NAME, packageName);

        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        try {
            data.writeInterfaceToken("com.iadb.server.IIadbBinderService");
            data.writeStrongBinder(IADB_APPLICATION.asBinder());
            data.writeInt(1);
            args.writeToParcel(data, 0);
            result = binder.transact(18 /*IIadbBinderService.Stub.TRANSACTION_attachApplication*/, data, reply, 0);
            reply.readException();
        } finally {
            reply.recycle();
            data.recycle();
    ...
    public static void onBinderReceived(@Nullable IBinder newBinder, String packageName) {
        ...
        if (newBinder == null) {
            ...
            binder = newBinder;
            service = IIadbBinderService.Stub.asInterface(newBinder);

            try {
                binder.linkToDeath(DEATH_RECIPIENT, 0);
            } catch (Throwable e) {
                if (BuildConfig.DEBUG) {
                    Log.i("IApplication", "attachApplication");
                }
            }

            try {
                boolean ret = attachApplicationV13(binder, packageName);
                if (BuildConfig.DEBUG) {
                    Log.i("IApplication", "attachApplication result " + ret);
                }
            } catch (Throwable e) {
                if (BuildConfig.DEBUG) {
                    Log.w("IApplication", Log.getStackTraceString(e));
                }
            }
        }
    }
     ...
     * <p>
     * Iadb APIs can only be used when the binder is received, or a
     * {@link IllegalStateException} will be thrown.
     *
     * <p>Note:</p>
     * <ul>
     * <li>The listener will be called in main thread.</li>
     * <li>The listener could be called multiply times. For example, user restarts Iadb when app is running.</li>
     * </ul>
     * <p>
     *
    ...
    @NonNull
    protected static IIadbBinderService requireService() {
        if (service == null) {
     ...
     * <p>
     * Use {@link IadbBinderWrapper} to wrap the original binder.
     *
     * @see IadbBinderWrapper
     */
    public static void transactRemote(@NonNull Parcel data, @Nullable Parcel reply, int flags) {
        try {
            requireService().asBinder().transact(IadbApiConstants.BINDER_TRANSACTION_transact, data, reply, flags);
     ...
     * requirements, use {@link Iadb#bindUserService(UserServiceArgs, ServiceConnection)}.
     * <p>
     ...
     * Use {@link Iadb#transactRemote(Parcel, Parcel, int)} for binder calls and {@link Iadb#bindUserService(UserServiceArgs, ServiceConnection)}
     * for complicated requirements.
     * <p>This method is planned to be removed from Iadb API 14.
     */
    private static IadbRemoteProcess newProcess(@NonNull String[] cmd, @Nullable String[] env, @Nullable String dir) {
        try {
            return new IadbRemoteProcess(requireService().newProcess(cmd, env, dir));
        } catch (RemoteException e) {
            throw rethrowAsRuntimeException(e);
        }
    }
    ...
    public static int getUid() {
        ...
        try {
            ...
            // Iadb pre-v11 and permission is not granted
            return -1;
        }
        return serverUid;
    }
    ...
    public static int getLatestServiceVersion() {
        return IadbApiConstants.SERVER_VERSION;
    }

    /**
     * Returns SELinux context of Iadb server process.
     *
     ...
     * If the user's su does not allow binder calls between su and app, Iadb will switch to context <code>u:r:shell:s0</code>.
     * </p>
     *
    ...
    public static class UserServiceArgs {
         ...
         * <br>Under daemon mode, the service will run forever until {@link Iadb#unbindUserService(UserServiceArgs, ServiceConnection, boolean)} is called.
         ...
         * the Iadb or Sui server can recreate the user service for you.
         *
        ...
        private Bundle forAdd() {
            ...
            options.putParcelable(IadbApiConstants.USER_SERVICE_ARG_COMPONENT, componentName);
            options.putBoolean(IadbApiConstants.USER_SERVICE_ARG_DEBUGGABLE, debuggable);
            options.putInt(IadbApiConstants.USER_SERVICE_ARG_VERSION_CODE, versionCode);
            options.putBoolean(IadbApiConstants.USER_SERVICE_ARG_DAEMON, daemon);
            options.putBoolean(IadbApiConstants.USER_SERVICE_ARG_USE_32_BIT_APP_PROCESS, use32BitAppProcess);
            options.putString(IadbApiConstants.USER_SERVICE_ARG_PROCESS_NAME,
                    Objects.requireNonNull(processName, "process name suffix must not be null"));
            if (tag != null) {
                options.putString(IadbApiConstants.USER_SERVICE_ARG_TAG, tag);
            }
            return options;
        }
        ...
        private Bundle forRemove(boolean remove) {
            ...
            options.putBoolean(IadbApiConstants.USER_SERVICE_ARG_REMOVE, remove);
            return options;
        }
    }
     ...
     * backend is Iadb and user starts Iadb with adb).
     * <p>
     ...
     * code for that method is {@code 57321} (use {@code 57320} in aidl).
     * In this method, you can do some cleanup jobs and call
     * {@link System#exit(int)} in the end.
     * <p>
     * If the backend is Iadb, whether in daemon mode or not, user service
     * will be killed when Iadb service is stopped or restarted.
     * Iadb sends binder to all Iadb apps. Therefore, you only need to
    ...
    public static void bindUserService(@NonNull UserServiceArgs args, @NonNull ServiceConnection conn) {
        IadbServiceConnection connection = IadbServiceConnections.get(args);
    ...
    /**
     * Similar to {@link Iadb#bindUserService(UserServiceArgs, ServiceConnection)},
     ...
     * For Iadb pre-v13, version is always 0 if service is running.
     * @see Iadb#bindUserService(UserServiceArgs, ServiceConnection)
    ...
    public static int peekUserService(@NonNull UserServiceArgs args, @NonNull ServiceConnection conn) {
        ...
        try {
            ...
            bundle.putBoolean(IadbApiConstants.USER_SERVICE_ARG_NO_CREATE, true);
    ...
    public static void unbindUserService(@NonNull UserServiceArgs args, @Nullable ServiceConnection conn, boolean remove) {
        if (remove) {
            ...
            /*
             * When unbindUserService remove=false is called, although the IadbServiceConnection
             * instance is removed from IadbServiceConnections, it still exists (since its a Binder),
             ...
             * This will not kill the service, but will remove the IadbServiceConnection instance
             * from the server.
             */
            try {
            ...
            IadbServiceConnections.remove(connection);
        }
    }
